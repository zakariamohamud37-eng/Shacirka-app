...
(async()=>{
  ...
  html=html.replace('</body>','<script src="native.js"></script><script src="native-glue.js"></script></body>');
  fs.writeFileSync('www/index.html',html);
  fs.cpSync('web-source/icons','www/icons',{recursive:true});
  fs.copyFileSync('native-glue.js','www/native-glue.js');
  await esbuild.build({entryPoints:['native.js'],bundle:true,format:'iife',outfile:'www/native.js'});
